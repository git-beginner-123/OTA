...
static volatile int s_master_volume_pct = 100;
static bool s_audio_out_ready = false;
static bool s_open_once_logged = false;
...
{
    if (s_engine_lock && s_audio_out_ready) return true;
    ...
    if (!s_engine_lock) return false;

    if (!s_audio_out_ready) {
        // Keep I2S driver resident to avoid repeated init/deinit churn.
        s_audio_out_ready = AudioOut_Init();
    }
    return s_audio_out_ready;
}

void AudioEngine_SetMasterVolumePercent(int volume_pct)
{
    s_master_volume_pct = clamp_pct(volume_pct);
}
                      ...
                      volatile bool* stop_flag)
{
    ...
    if (!AudioEngine_Init() || !s_engine_lock || !s_audio_out_ready) return false;
    ...
    int effective = (volume_pct * s_master_volume_pct) / 100;
    AudioOut_SetVolumePercent(effective);

    session->open = true;
    session->source = source;
    session->volume_pct = volume_pct;
    session->stop_flag = stop_flag;
    if (!s_open_once_logged) {
        ESP_LOGI(TAG, "audio out persistent mode active");
        s_open_once_logged = true;
    }
    return true;
}
