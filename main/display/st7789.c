...
#define LCD_CTRL_LEGACY_ST77XX 0
#define LCD_CTRL_ILI9341       1

// Default to ILI9341 compatibility mode.
// Set to LCD_CTRL_LEGACY_ST77XX for the previous minimal init sequence.
#ifndef LCD_PANEL_CTRL
#define LCD_PANEL_CTRL LCD_CTRL_ILI9341
#endif
...
#define ST7789_SW_INVERT_DEFAULT 0
#define ST7789_SW_RB_SWAP_DEFAULT 0
#define ST7789_HW_INVERT_DEFAULT 0

static bool s_sw_invert = ST7789_SW_INVERT_DEFAULT;
static bool s_sw_rb_swap = ST7789_SW_RB_SWAP_DEFAULT;
static bool s_hw_invert = ST7789_HW_INVERT_DEFAULT;
...
#if LCD_PANEL_CTRL == LCD_CTRL_LEGACY_ST77XX
static void lcd_init_legacy(void)
{
    write_cmd(0x01);
    ...
    write_cmd(ST7789_HW_INVERT_DEFAULT ? 0x21 : 0x20); // Inversion ON/OFF
    write_cmd(0x29); // Display on
}
#endif

#if LCD_PANEL_CTRL == LCD_CTRL_ILI9341
static void lcd_init_ili9341(void)
{
    // Sequence based on common ILI9341 240x320 SPI bring-up.
    ...
    {
        // ILI9341 on this board needs MX set to avoid left-right mirrored text.
        uint8_t d = 0x48; // MX | BGR, rotation = 0
        write_data(&d, 1);
    }
    write_cmd(0x3A); // COLMOD
    {
        uint8_t d = 0x55; // RGB565
        write_data(&d, 1);
    }
    ...
    vTaskDelay(pdMS_TO_TICKS(120));
    write_cmd(0x13); // Normal display mode
    write_cmd(ST7789_HW_INVERT_DEFAULT ? 0x21 : 0x20);
    write_cmd(0x29); // Display on
}
#endif
...
// -----------------------------
#define LCD_DMA_QUEUE        3
#define LCD_DMA_CHUNK_BYTES  8192  // must be <= max_transfer_sz
...
// Public API
// -----------------------------
int St7789_Width(void)  { return ST7789_W; }
int St7789_Height(void) { return ST7789_H; }
...
void St7789_Init(void)
{
    ...
    s_sw_invert = ST7789_SW_INVERT_DEFAULT;
    s_sw_rb_swap = ST7789_SW_RB_SWAP_DEFAULT;
    ...
    ESP_LOGI(kTag, "init done ctrl=%s spi=%u Hz, dma_q=%d, dma_chunk=%d",
             (LCD_PANEL_CTRL == LCD_CTRL_ILI9341) ? "ILI9341" : "legacy",
             (unsigned)LCD_SPI_HZ, LCD_DMA_QUEUE, LCD_DMA_CHUNK_BYTES);
}
...
void St7789_BlitRect(int x, int y, int w, int h, const uint16_t* pixels565)
{
    if (w <= 0 || h <= 0) return;
    if (x < 0 || y < 0) return;
    if (x + w > ST7789_W) return;
    if (y + h > ST7789_H) return;
    if (!pixels565) return;
...
void St7789_Fill(uint16_t color565)
{
    ...
    set_addr_window(0, 0, ST7789_W - 1, ST7789_H - 1);
    lcd_dma_queue_color565(color565, ST7789_W * ST7789_H);
...
void St7789_SetSoftwareInvert(bool on) { s_sw_invert = on; }
void St7789_SetSoftwareRBSwap(bool on) { s_sw_rb_swap = on; }
bool St7789_GetSoftwareInvert(void) { return s_sw_invert; }
bool St7789_GetSoftwareRBSwap(void) { return s_sw_rb_swap; }

void St7789_ApplyPanelDefaultProfile(void)
{
    s_sw_invert = ST7789_SW_INVERT_DEFAULT;
    s_sw_rb_swap = ST7789_SW_RB_SWAP_DEFAULT;
    St7789_SetInversion(ST7789_HW_INVERT_DEFAULT);
}
bool St7789_GetInversion(void) { return s_hw_invert; }
